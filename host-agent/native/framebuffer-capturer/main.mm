...
@property (nonatomic, assign) BOOL sentConfig;
@end

static void WriteAnnexB(CMSampleBufferRef sampleBuffer, BOOL *sentConfig) {
...
static void CompressionCallback(void *outputCallbackRefCon, void *sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags flags, CMSampleBufferRef sampleBuffer) {
  ...
  BOOL sent = encoder.sentConfig;
  WriteAnnexB(sampleBuffer, &sent);
  encoder.sentConfig = sent;
}
